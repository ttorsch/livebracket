...
// (court, planned_time, scheduled_time, pinned, referee_team_id) — and
// matches.round_id cascades on delete. So a redraw does not orphan a
// division's placements, it deletes them, and nothing survives afterwards to
// report. The only honest moment to count them is before the delete, which is
...
export interface DiscardCost {
  ...
  placed: number;
  /** Placements the organizer fixed by hand. */
  pinned: number;
  /** Referee duty, assigned by hand and keyed the same way. */
  refereed: number;
}

export const NO_DISCARD_COST: DiscardCost = { placed: 0, pinned: 0, refereed: 0 };
...
export function tallyDiscardCost(rows: readonly PlacementRow[]): DiscardCost {
  let placed = 0, pinned = 0, refereed = 0;
  for (const m of rows) {
    ...
    // A pin is organizer work whether or not the pinned match ended up placed,
    // so it is counted on its own terms rather than as a subset of `placed`.
    if (m.pinned) pinned++;
    if (m.referee_team_id) refereed++;
  }
  return { placed, pinned, refereed };
}

/** Nothing here is worth warning anyone about. */
export function isEmptyCost(c: DiscardCost): boolean {
  return c.placed === 0 && c.pinned === 0 && c.refereed === 0;
}

/** "46 scheduled matches, 14 manual edits and 3 referee assignments" — real
 *  counts, never naming a category that is zero, so the sentence stays true
 *  for a division that has times but no hand edits. */
export function describeDiscardCost(cost: DiscardCost): string {
  ...
  if (cost.pinned > 0) parts.push(`${cost.pinned} manual edit${cost.pinned === 1 ? '' : 's'}`);
